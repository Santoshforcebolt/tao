...
class OrdersViewController: BaseViewController<OrdersViewModel> {
    ...
    func setupNavigationBar() {
        ...
        let navigationItem = UINavigationItem(title: "My Orders")
...
extension OrdersViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.viewModel.orderList?.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let order = self.viewModel.orderList?[indexPath.row],
           let urlString = order.productImage,
           let url = URL(string: urlString) {
            let (color,subtitleString) = self.viewModel.getStatusString(for: indexPath.row)
            cell.setup(image: .withURL(url),
                       title: order.productName ?? "",
                       price: "\(order.amount ?? 0)",
                       subtitle: subtitleString)
            cell.updateSubtitleColor(color: color)
        }
        return cell
    }
