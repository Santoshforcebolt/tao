//
//  CategoryCompetitionViewController.swift
//  tao
//
...
class CategoryCompetitionViewController: BaseViewController<CategoryCompetitionViewModel> {
    var navigationBar: UINavigationBar?
    var transactionCollectionView: UICollectionView
   
    override init(_ viewModel: CategoryCompetitionViewModel) {
...
extension CategoryCompetitionViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.viewModel.competitions?.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let compUrl = URL(string: competition?.imageURL ?? "") {
            ...
            if competition?.rewardType ?? "COIN" == "COIN" {
                cell.setupData(compImage: .withImage(UIImage(named: "defaultLeaderboardPicture")!),
                               sponsorImage: competition?.guide?[0].image,
                               nudgeType: .coin("\(competition?.prize ?? 0)"),
                               daysRemaining: days > 0 ? TextType.plain("\(days)"): TextType.clock(endDate))
            } else {
