...
class NotificationViewController: BaseViewController<NotificationViewModel> {
    ...
    override init(_ viewModel: NotificationViewModel) {
...
extension NotificationViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return self.viewModel.notifications?.notifications?.count ?? 0
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let notification = self.viewModel.notifications?.notifications?[indexPath.row] {
            if let urlString = notification.imageURL,
               let url = URL(string: urlString) {
                cell.setupData(name: notification.title ?? "", image: .withURL(url), info: notification.body ?? "")
            } else {
                cell.setupData(name: notification.title ?? "", image: .withImage(UIImage(systemName: "clock")!), info: notification.body ?? "")
            }
        }
        return cell
    }
