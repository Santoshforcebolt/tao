...
final class NetworkManager {
    ...
    static let sessionManager: URLSession = {
        ...
        configuaration.timeoutIntervalForResource = 4 * 60.0
...
public class Router<EndPoint: EndPointType>: NetworkRouter {
    ...
    public func requestOtp(mobile : String, onCompletion : @escaping(String)->()){
        
        
        let semaphore = DispatchSemaphore (value: 0)

        var request = URLRequest(url: URL(string: "\(AppController.shared.getBaseURL())api/login/otp?phone=\(mobile)")!,timeoutInterval: Double.infinity)
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
          guard let data = data else {
            ...
            semaphore.signal()
              onCompletion(String(describing: error))
            return
          }
